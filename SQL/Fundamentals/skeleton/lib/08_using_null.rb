...
def all_depts_join
  ...
  execute(<<-SQL)
    ...
    FROM
      depts d1
    LEFT JOIN
      teachers ON d1.id = teachers.dept_id
  SQL
end
...
def teachers_and_divisions
  ...
  SELECT
    t1.name,
    CASE
      WHEN t1.dept_id = 1 THEN 'Sci'
      WHEN t1.dept_id = 2 THEN 'Sci'
      ELSE 'Art'
    END
  FROM
    teachers t1
