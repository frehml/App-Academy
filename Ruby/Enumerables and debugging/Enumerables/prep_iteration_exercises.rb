  ...
  def bubble_sort!(&prc)
    ...
    while sorted != 1
      ...
      self.each.with_index do |num, idx|
          if !self[idx+1].nil? && prc.call(self[idx], self[idx+1]) < 0
...
def substrings(string)
  ...
  while x < string.length
    new_array << string[x..y]
    y += 1 if x == 0
    x += 1 if y == string.length 
  end
  return new_array
end
...
def subwords(word, dictionary)
  ...
  substrings.each { |word| new_array << word if dictionary.include?(word)} 
  return new_array
end
...
class Array
  def my_map(&prc)
    new_array = []
    self.my_each { |num| new_array << proc.call(num)}
    return new_array
  end
  ...
  def my_inject(&blk)
    array = []
    self.each do |ele|
      array.empty? ? array << ele : array[0] = blk.call(array[0], ele)
    end
    return array
  end
end
