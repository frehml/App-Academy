...
def sum_array(array)
    ...
    sum = array[0] + sum_array(array[1..-1])
...
def reverse_string(str)
    ...
    new_word = ""
    return new_word if new_word.length == str.length

    new_word = str[-1] + reverse_string(str[0...(str.length - 1)])
...
def flatten(data)
    ...
    return [data] if data.is_a?(String) || data.is_a?(Integer)
